...
export const getDepartment = (num_dep: string): Department => {
  return map.get(num_dep) ?? na;
};
...
const departements_regions: Department[] = [
  {
    num_dep: '01',
    dep_name: 'Ain',
...
const getColor = (region: string): string => {
  return colors[regions.findIndex((name) => name === region)];
};

departements_regions.forEach((department) =>
  map.set(department.num_dep, { ...department, color: getColor(department.region_name) }),
);
